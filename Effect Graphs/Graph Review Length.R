...
graph_data_plot <- graph_data_filter_days %>% group_by(bins) %>% summarise(mean = mean(word_count))
View(graph_data_plot)

pdf(file = "../Plots/review_length.pdf")
plot1 <- ggplot(data=graph_data_plot, aes(x=bins, y=mean, group=1))+
    geom_line() + 
