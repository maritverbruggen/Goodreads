...
graph_data_plot <- graph_data_filter_days %>% group_by(treated, days_since_gw_bin) %>% count()
View(graph_data_plot)

pdf(file = "../Plots/#Ratings.pdf")
ggplot(data=graph_data_plot, aes(x=days_since_gw_bin, y=n, group=treated, colour=treated))+
  geom_line() + 
...
ggplot(data=ra_gw_graph, aes(x=bins, y=n, group=1))+
  geom_line() + 
  geom_vline(xintercept = "(0,30]", 
             color = "blue") +
  theme(text = element_text(size=10),
